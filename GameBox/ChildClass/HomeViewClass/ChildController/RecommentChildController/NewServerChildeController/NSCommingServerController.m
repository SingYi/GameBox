...
@interface NSCommingServerController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation NSCommingServerController
...
#pragma mark - method
- (void)refreshData {
    [GameRequest CommingSoonServerOpenWithPage:@"1" Completion:^(NSDictionary * _Nullable content, BOOL success) {
//        syLog(@"%@",content);
...
- (void)loadMoreData {
    if (_isAll || _currentPage == 0) {
        ...
        _currentPage++;
        [GameRequest CommingSoonServerOpenWithPage:[NSString stringWithFormat:@"%ld",_currentPage] Completion:^(NSDictionary * _Nullable content, BOOL success) {
            if (success && REQUESTSUCCESS) {
