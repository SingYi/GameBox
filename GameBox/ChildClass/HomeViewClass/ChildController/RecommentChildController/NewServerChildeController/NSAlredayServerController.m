...
@interface NSAlredayServerController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation NSAlredayServerController
...
#pragma mark - method
- (void)refreshData {
    [GameRequest AlredayServerOpenWithPage:@"1" Completion:^(NSDictionary * _Nullable content, BOOL success) {
//        syLog(@"%@",content);
...
- (void)loadMoreData {
    if (_isAll || _currentPage == 0) {
        ...
        _currentPage++;
        [GameRequest AlredayServerOpenWithPage:[NSString stringWithFormat:@"%ld",_currentPage] Completion:^(NSDictionary * _Nullable content, BOOL success) {
            if (success && REQUESTSUCCESS) {
